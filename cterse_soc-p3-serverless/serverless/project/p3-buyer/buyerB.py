...
with open("p3protocol2.json") as stream:
    protocol = json.load(stream)
with open(os.environ['CONFIG']) as conf:
    configuration = yaml.safe_load(conf)

adapter = pos.Adapter(name, protocol, configuration, 'BuyerB')
...
def send_quote(event, context):
    for record in event["Records"]:
        ...
        if quote_req["buyerName"] == "buyerA":
            print("Quote to be sent by Buyer A. Discarded by Buyer B")
            return 
