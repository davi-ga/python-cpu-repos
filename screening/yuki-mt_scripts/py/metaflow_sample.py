...
items_path = os.environ.get("ITEMS_PATH", "data/items.csv")
test_path = os.environ.get("TEST_PATH", "data/test.csv")
sales_path = os.environ.get("SALES_PATH", "data/sales_train.csv")
...
class SalesFlow(FlowSpec):
    ...
    def grid(self):
        ...
        for block_num in self.df["date_block_num"].unique():
            cur_shops = self.df[self.df["date_block_num"] == block_num][
                "shop_id"
            ].unique()
            cur_items = self.df[self.df["date_block_num"] == block_num][
                "item_id"
            ].unique()
            grid.append(
                np.array(
    ...
    def group(self):
        ...
        if name != "shop":
            self.df[f"{name}_price"] = gb.item_price.agg(
                {"item_price": "mean"}
            ).item_price
        self.df[f"{name}_sales_price"] = gb.sales_price.agg(
            {"sales_price": "sum"}
        ).sales_price
        self.cols = cols
    ...
    def mean_encoding(self):
        ...
        for train_index, test_index in kf.split(X, y):
            ...
            feats.append(
                test_df[target_col].map(train_df.groupby(target_col).target.mean())
            )
    ...
    def build_test_data(self):
        self.test_df = pd.merge(
            pd.read_csv(test_path), pd.read_csv(items_path), on="item_id"
        )
    ...
    def pricing_map(self):
        ...
        test_shop_item_s = (
            self.test_df.shop_id.astype(str) + "-" + self.test_df.item_id.astype(str)
        )
        df["both_price"] = test_shop_item_s.map(shop_item_map)
        df.loc[df.both_price.isnull(), "both_price"] = self.test_df.item_id.map(
            item_map
        )
        df.loc[df.both_price.isnull(), "both_price"] = (
            self.test_df.item_category_id.map(category_map)
        )

        df["item_price"] = self.test_df.item_id.map(item_map)
        df.loc[df.item_price.isnull(), "item_price"] = (
            self.test_df.item_category_id.map(category_map)
        )
    ...
    def lag(self):
        ...
        self.cols_to_rename = list(
            self.df.columns.difference(grid_cols + self.lag_drop_cols)
        )
    ...
    def remove_cols(self):
        lag_cols = [
            ...
            if col[-1] in [str(item) for item in self.shift_range]
        ]

        keep_cols = (
            ["ID"]
            + list(set(grid_cols) - set(["date_block_num"]))
            + self.lag_drop_cols
            + lag_cols
        )
